...
export const problemSlice=createSlice({
   ...
   reducers:{
    ...
    get_Single_problem:(state,action)=>{
      ...
      const updatedProblems = state.problems.map((itm)=>(itm._id===action.payload._id?action.payload:itm));
      const updatedSingle = Array.isArray(state.single_recent_post)
        ? state.single_recent_post.map((itm)=>(itm._id===action.payload._id?action.payload:itm))
        : (state.single_recent_post?._id===action.payload._id ? action.payload : state.single_recent_post);
      return {...state,problems:updatedProblems,single_recent_post:updatedSingle};
    },
