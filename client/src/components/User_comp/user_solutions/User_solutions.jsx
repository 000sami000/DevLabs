...
import { FiBookmark, FiCheckCircle, FiMessageSquare, FiSearch, FiStar } from "react-icons/fi";
import { useNavigate } from "react-router-dom";
import {
  fetch_allSolutions,
  fetch_savedSolutions,
  fetch_userSolutions,
  search_savedSolution,
  search_userSolution,
} from "../../../api";
...
const normalizeSolution = (item = {}) => ({
  _id: item._id,
  p_id: item.p_id || item.problem_id || null,
  p_title: item.p_title || "Solution",
  p_creator_username: item.p_creator_username || "",
  createdAt: item.createdAt,
  vote: Number(item.vote) || 0,
  total_comments: Number(item.total_comments) || 0,
  isApproved: item.isApproved !== false,
  profile_img_: item.profile_img_ || null,
});

function User_solutions({ mode = "user" }) {
  const navigate = useNavigate();
  const isAdmin = mode === "admin";

  const tabs = useMemo(
    () => [
      { key: "mine", label: "My Solutions", icon: <FiCheckCircle /> },
      { key: "saved", label: "Saved", icon: <FiBookmark /> },
      ...(isAdmin ? [{ key: "platform", label: "All Solutions", icon: <FiCheckCircle /> }] : []),
    ],
    [isAdmin]
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (activeTab === "saved") {
          response = query ? await search_savedSolution(query) : await fetch_savedSolutions();
        } else if (activeTab === "platform" && isAdmin) {
          response = await fetch_allSolutions(query || "");
        } else {
          response = query ? await search_userSolution(query) : await fetch_userSolutions();
        }
        ...
        if (!ignore) {
          setItems(rows.map(normalizeSolution));
        }
      } catch (requestError) {
        if (!ignore) {
          setItems([]);
          setError(requestError?.response?.data?.message || requestError?.message || "Unable to load solutions.");
        }
      } finally {
        if (!ignore) {
          setLoading(false);
          ...
          >
            <div className="theme-input flex flex-1 items-center gap-2 px-3 py-2">
              ...
              <input
                ...
                placeholder="Search by related problem title"
      ...
      <div className="theme-surface border overflow-hidden">
        <div className="flex items-center justify-between border-b border-[var(--app-border)] px-4 py-3">
          <div className="text-sm font-semibold">
            {tabs.find((tab) => tab.key === activeTab)?.label || "Solutions"}
          </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-[var(--app-border)]">
            {currentItems.map((item) => (
              <button
                key={item._id}
                type="button"
                onClick={() => navigate(item.p_id ? `/problem/${item.p_id}/sols?s_id=${item._id}` : "/community")}
                className="w-full px-4 py-4 text-left hover:bg-[var(--app-bg-soft)]"
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="min-w-0 flex-1">
                    <h4 className="truncate text-base font-semibold text-[var(--app-text)]">{item.p_title}</h4>
                    <div className="theme-text-muted mt-2 flex flex-wrap items-center gap-3 text-xs">
                      <span className="inline-flex items-center gap-1 text-[#1a7f37]"><FiStar /> {item.vote}</span>
                      <span className="inline-flex items-center gap-1"><FiMessageSquare /> {countValue(item.total_comments)} comments</span>
                      <span>{item.createdAt ? formatDistanceToNow(new Date(item.createdAt), { addSuffix: true }) : "recently"}</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-3">
                    {item.p_creator_username ? (
                      <div className="inline-flex items-center gap-2">
                        <img
                          src={getAssetUrl(item.profile_img_)}
                          alt={item.p_creator_username}
                          className="h-8 w-8 rounded-full object-cover"
                        />
                        <span className="theme-text-muted text-xs">@{item.p_creator_username}</span>
                      </div>
                    ) : null}
          ...
          <div className="theme-text-muted px-4 py-10 text-center text-sm">No solutions found.</div>
        )}
      </div>
